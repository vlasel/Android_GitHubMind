...
public class MainActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getService().zen(new Callback<Response>() {
            @Override
            public void success(Response response, Response response2) {
                try {
                    byte[] buffer = new byte[response.getBody().in().available()];
                    response.getBody().in().read(buffer);
                    messageView.setText(new String(buffer, Charset.forName("utf-8")));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
